...
const basicSalaryList = async (req, res) => {
    try {
        const basicSalaryList = await basicSalaryModel.find({}).populate('user', userDataQuery);
...
const deleteBasicSalary = async (req, res) => {
    try {
        ...
        const dataList = await basicSalaryModel.find({}).populate('user', userDataQuery);
        return res.status(201).json({
            success: true,
...
//     });
//
//     const reports = await salaryReportModel.find(filters).populate('user', userDataQuery);
//
...
const allTypeWiseSalaryReports = async (query) => {
    ...
    const [allBasicSalaries, appSettings, allHolidays, allUserLeaves, allUserPunchReports] = await Promise.all([
        basicSalaryModel.find(undefined, undefined, undefined).populate('user', userDataQuery).lean(),
    ...
    // const allBasicSalaries = await basicSalaryModel.find(undefined, undefined, undefined).populate('user', userDataQuery);
    ...
    try {
        ...
        for (const punch of addPunch) {
            ...
            await punchReportModel.updateOne(
                ...
                {
                    $set: {
                        "punchReport.$.isDeductible": true
                    }
                }
            );
        }

        for (const leave of addLeave) {
            await LeaveModel.updateOne(
                { _id: leave._id },
                {
                    $set: {
...
const generateTypeWiseReport = async (query) => {
    ...
    const [allBasicSalaries, appSettings, allHolidays] = await Promise.all([
        basicSalaryModel.find().populate('user', userDataQuery).lean(),
        AppSettingModel.findOne().lean(),
    ...
    const reports = await salaryReportModel.find(filters).populate('user', userDataQuery).lean();
