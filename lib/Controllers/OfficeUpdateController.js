const {handleError, generateRandomId, UserRole, userDataQuery} = require("../utils/utils");
const {OfficeUpdateModel} = require("../Models/OfficeUpdateModel");

const getOfficeUpdateList = async () => {
    return OfficeUpdateModel.find().populate('createdBy', userDataQuery);
};
    ...
    try {
        ...
        if (id) {
            const officeUpdate = await OfficeUpdateModel.findById(id)
                .populate('createdBy', userDataQuery);
