const { handleError, UserRole } = require("../utils/utils");
...
const updateApp = async (req, res) => {
    try {
        ...
        // Restrict updates to admin or creator
        if (
            loginUser.role !== UserRole.Admin &&
...
const getAllApps = async (req, res) => {
    try {
        ...
        return res.status(200).json({
            ...
            data: versionList,
            mongoData: process.env.MONGO_CONN
        });
    } catch (err) {
        return res.status(500).json({
            success: false,
...
const getSingleApp = async (req, res) => {
    try {
        ...
        // Restrict access to admin or creator
        if (
            loginUser.role !== UserRole.Admin &&
            version.createdBy.toString() !== loginUser._id.toString()
        ) {
...
const isUpdateAvailable = async (req, res) => {
    try {
        const { currentVersion } = req.query; // Current version sent by the client
        const loginUser = req.user;
