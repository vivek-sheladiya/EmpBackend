...
const calculateAttendanceStats = async (attendance) => {
  ...
  let totalHours = attendance.punchTime.reduce((sum, p) => {
  ...
  let breakHours = attendance.breakTime.reduce((sum, b) => {
  ...
  let lateArrival = attendance.punchInAt > officeHoursStart ? attendance.punchInAt - officeHoursStart : 0;

  let earlyArrival = attendance.punchInAt < officeHoursStart ? officeHoursStart - attendance.punchInAt : 0;
  ...
  try {
    ...
    if (id) {
      ...
      const newTask = new TasksModel({
        ...
        taskAssignee: req.body.taskAssignee.map(user => ({ userId: user._id })),
        taskLabels: req.body.taskLabels,
        taskStartDate: req.body.taskStartDate,
        taskEndDate: req.body.taskEndDate,
        taskEstimatedTime: req.body.taskEstimatedTime,
        taskAttachments: req.body.taskAttachments,
        taskAddedBy: req.body.taskAddedBy,
      });
