...
const addAttendance = async (req, res) => {
  ...
  try {
    ...
    if (punchInTime) {
      punchInTime = isOffline ? req.body.punchInTime : Date.now();
    }
    if (punchOutTime) {
...
const updateTask = async (req, res) => {
  try {
    ...
    if (updateData.taskTitle && updateData.taskTitle !== existingTask.taskTitle) {
      historyChanges.push({
        fieldName: "taskTitle",
        oldValue: existingTask.taskTitle,
        newValue: updateData.taskTitle,
        changedBy: req.body.userId,
        changeTime: new Date(),
      });
    }

    if (updateData.taskStatus && updateData.taskStatus !== existingTask.taskStatus) {
      historyChanges.push({
        fieldName: "taskStatus",
        oldValue: existingTask.taskStatus,
        newValue: updateData.taskStatus,
        changedBy: req.body.userId,
        changeTime: new Date(),
      });
    }
