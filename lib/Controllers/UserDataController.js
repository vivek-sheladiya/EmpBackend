...
const deleteScreenshot = async (req, res) => {
  try {
    ...
    const screenshot = attendance.screenshots.find((ss) => ss._id.toString() === ssid);
    if (!screenshot) {
    ...
    if (!imageUrl) {
      return res.status(400).json({ success: false, message: "Image URL not found in record" });
    }
    ...
    attendance.screenshots = attendance.screenshots.filter((ss) => ss._id.toString() !== ssid);
    await attendance.save();
