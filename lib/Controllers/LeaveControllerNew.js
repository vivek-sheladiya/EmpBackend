...
const {transporter} = require("../utils/smtp_mailer");
...
const addUpdateLeave = async (req, res) => {
    try {
        ...
        if (leaveBody.leaveCategory === leaveLabelKeys.paid) {
            const result = await getRemainingLeave(leaveBody);
        ...
        if (leaveBody.leaveType === leaveLabelKeys.fullDay) {
            ...
            if (leaveBody.dayType === leaveLabelKeys.multipleDay) {
                ...
                if (leaveBody.leaveCategory === leaveLabelKeys.paid && totalDays > monthlyLimit) {
        ...
        if (leaveBody.leaveType !== leaveLabelKeys.manualHours) {
            ...
            const leaveStartStr = startDate.format("YYYY-MM-DD");
            const leaveEndStr = endDate.format("YYYY-MM-DD");

            // if (isSunday(startDate) || isSunday(endDate)) {
            //     isSandwichLeave = true;
            // }

            // const beforeStart = moment(startDate).subtract(1, 'day');
            // const afterEnd = moment(endDate).add(1, 'day');

            // if (isSunday(beforeStart) || isSunday(afterEnd)) {
            //     isSandwichLeave = true;
            // }
        ...
        if (id) {
            ...
            const updatedLeave = await leaveModel.findById(id);

            const list = await leaveModel.find({}).populate('user', 'fullName');
            const leaveList = list.map(({_doc, user}) => ({
                ..._doc,
                user: {
                    userId: user?._id,
            ...
            //
            // const mailOptions = {
            //     from: "Keyur " + process.env.SMTP_USER,
