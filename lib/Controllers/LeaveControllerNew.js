...
const addUpdateLeave = async (req, res) => {
    try {
        ...
        let leaveList = [];
        const user = await UserModel.findOne({ _id: userID, role: 'Employee' });
        if (user) {
            const empLeaves = await leaveModel.find({ user: userID }).populate('user', 'fullName');
            leaveList = formatLeaves(empLeaves);
        } else {
            const allLeaves = await leaveModel.find({}).populate('user', 'fullName');
            leaveList = formatLeaves(allLeaves);
        }
