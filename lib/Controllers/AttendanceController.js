...
const addAttendance = async (req, res) => {
    ...
    try {
        ...
        let isOfflineData = req.body.isOfflineData || false;
        ...
        if (!isOfflineData && punchInTime) {
            punchInTime = Date.now();
        }
        if (!isOfflineData && punchOutTime) {
            punchOutTime = Date.now();
        }
        if (!isOfflineData && breakInTime) {
            breakInTime = Date.now();
        }
        if (!isOfflineData && breakOutTime) {
            breakOutTime = Date.now();
        }

        const settings = await AppSettingModel.findOne();

        const officeHoursStart = dayjs(settings.appSettings.officeStartTime, "h:mm a").valueOf();
        const officeHoursEnd = dayjs(settings.appSettings.officeEndTime, "h:mm a").valueOf();
        const breakDuration = settings.appSettings.breakDuration * 60 * 1000;
        const totalOfficeHourMilli = dayjs(settings.appSettings.officeEndTime, "h:mm a").diff(dayjs(settings.appSettings.officeStartTime, "h:mm a"), "milliseconds") - breakDuration;
        ...
        console.log("mouseEventCount=>", mouseEventCount)
        ...
        if (screenshot) {
            ...
            form.append("file", blob, `${Date.now()}_${screenshot.originalname}`);
            form.append("type", 'screenshots');

            const response = await fetch(`${environment.apiBaseUrl}uploadFile.php`, {
                method: "POST",
                body: form,
            });
...
const calculateAttendanceStats = async (attendance) => {
    ...
    const officeHoursStart = dayjs(`${attendanceDate.toISOString().split('T')[0]} ${settings.appSettings.officeStartTime}`, "YYYY-MM-DD h:mm a").valueOf();
    const officeHoursEnd = dayjs(`${attendanceDate.toISOString().split('T')[0]} ${settings.appSettings.officeEndTime}`, "YYYY-MM-DD h:mm a").valueOf();
    const breakDuration = settings.appSettings.breakDuration * 60 * 1000;
    const totalOfficeHourMilli = officeHoursEnd - officeHoursStart;

    let latestAttendance = attendance;

    if (attendance.punchInAt && attendance.punchInAt < todayStart) {
        if (attendance.isPunchIn) {
            attendance.isUnderVerification = true;
            attendance.isPunchIn = false;

            if (Array.isArray(attendance.punchTime) && attendance.punchTime.length > 0) {
                const lastPunch = attendance.punchTime[attendance.punchTime.length - 1];
                lastPunch.punchOutTime = officeHoursEnd;
            }

            if (attendance.isBreakIn && Array.isArray(attendance.breakTime) && attendance.breakTime.length > 0) {
                const lastBreak = attendance.breakTime[attendance.breakTime.length - 1];
    ...
    let totalHours = Array.isArray(latestAttendance.punchTime)
        ? latestAttendance.punchTime.reduce((sum, p) => {
    ...
    let breakHours = Array.isArray(latestAttendance.breakTime)
        ? latestAttendance.breakTime.reduce((sum, b) => {
    ...
    let lateArrival = latestAttendance.punchInAt > officeHoursStart ? latestAttendance.punchInAt - officeHoursStart : 0;

    let earlyArrival = latestAttendance.punchInAt < officeHoursStart ? officeHoursStart - latestAttendance.punchInAt : 0;
...
const deleteScreenshot = async (req, res) => {
    try {
        ...
        try {
            ...
            const response = await fetch(`${environment.apiBaseUrl}deleteFile.php`, {
                method: "POST",
                body: form,
            });
        ...
        res.json({success: true, message: "Screenshot deleted successfully", data: attendance});
    } catch (error) {
