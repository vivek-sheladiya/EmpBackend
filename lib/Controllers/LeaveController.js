...
const addLeave = async (req, res) => {
    try {
        ...
        if (leaveCategory === leaveLabelKeys.paid) {
            ...
            if (paidLeavesThisMonth.length === settings.monthlyMaxPaidLeave) {
            ...
            if (paidLeavesThisYear.length === settings.yearlyPaidLeave) {
        ...
        if (leaveCategory) {
            ...
            if (diffInDays <= 1) {
                ...
                leaveData.leaveCategory = leaveLabelKeys.unpaid;
            } else {
        ...
        const starts = moment(startDate, moment.ISO_8601, true);
        const ends = moment(endDate, moment.ISO_8601, true);
        const totalDays = ends.diff(starts, 'days') + 1;
        console.log("totalDays", totalDays)
        if (leaveData.leaveType === leaveLabelKeys.fullDay || leaveData.dayType === leaveLabelKeys.multipleDay) {
            const html = await ejs.renderFile("emailtemplates/leaveAddedFullDayMultiDay.ejs", {
                userName: users.fullName,
                leaveType: leaveData.leaveType,
                totalDays: totalDays,
                startDate: leaveData.startDate,
        ...
        const list = await leaveModel.find({})
        return res.status(201).json({ success: true, message: "Leave Added Successfully.", list });
    } catch (err) {
        return res.status(500).json({
            success: false,
            message: err.message,
