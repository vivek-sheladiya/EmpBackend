    ...
    try {
        ...
        if (leaveList.leaveType === leaveLabelKeys.fullDay) {
            ...
            mailer.sendmail({
                ...
                subject: `Leave Application Status Update - ${fromCamelCaseToLabel(leaveList.status)}`,
                // text: 'welcome to nodemailer',
                text: `Hi ${fromCamelCaseToLabel(user.fullName)},\n\n` +
                    `Your ${fromCamelCaseToLabel(leaveList.leaveType)} leave on ${formattedDate} ` +
                    `is now **${fromCamelCaseToLabel(leaveList.status)}**.\n\n` +
