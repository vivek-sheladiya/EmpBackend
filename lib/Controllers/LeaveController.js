...
async function checkLeave(user, startDate, category, limit, leaveModel, type = 'monthly') {
...
function checkUnexpectedLeave(startDate, requestedCategory) {
    ...
    console.log(start, today, diffInDays, isUnexpected);
    return {
        isUnexpected,
...
const addLeave = async (req, res) => {
    try {
        ...
        if (leaveCategory === leaveLabelKeys.paid) {
            const yearlyAllowed = await checkLeave(user, startDate, leaveLabelKeys.paid, settings.appSettings.yearlyPaidLeave, leaveModel, 'yearly');
            if (!yearlyAllowed) {
                return handleError(res, `Only ${settings.appSettings.yearlyPaidLeave} paid leaves allowed per year.`, 400);
            }
            const monthlyAllowed = await checkLeave(user, startDate, leaveLabelKeys.paid, settings.appSettings.monthlyMaxPaidLeave, leaveModel, 'monthly');
            if (!monthlyAllowed) {
                return handleError(res, `Only ${settings.appSettings.monthlyMaxPaidLeave} paid leaves allowed per month.`, 400);
            }
        }
        ...
        if (leaveData.dayType === leaveLabelKeys.singleDay) {
            query.startDate = _startDate;
        } else {
            query.startDate = { $lte: _endDate };
            query.$or = [
        ...
        const leaveAddedSameDate = await leaveModel.find(query);
        console.log("leaveAddedSameDate", leaveAddedSameDate);
        if (leaveAddedSameDate.length > 0) {
...
const updateLeave = async (req, res) => {
    try {
        ...
        const body = req.body;
        const { isUnexpected, sandwichLeave, leaveType, leaveCategory, startDate, endDate, reason, leaveHalfDayType, dayType, startTime, endTime, user } = body;
        if (!user) {
            return handleError(res, "User is required.", 400);
        }
        ...
        const _startDate = leaveData.startDate;
        const _endDate = leaveData.endDate || _startDate;
        const leaveAddedSameDate = await leaveModel.find({
            user,
            _id: { $ne: _id },
        ...
        const existingDate = moment(existingLeave.startDate).format('YYYY-MM-DD');
        const leaveDate = moment(leaveData.startDate).format('YYYY-MM-DD');
        if (req.body.sandwichLeave) {
            leaveData.sandwichLeave = req.body.sandwichLeave;
        } else if (existingDate !== leaveDate) {
            leaveData.sandwichLeave = await detectSandwichLeave(
                leaveData.startDate,
                leaveData.endDate,
            );
