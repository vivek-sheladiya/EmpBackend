...
const addLeave = async (req, res) => {
    try {
        ...
        if (leaveCategory === leaveLabelKeys.paid) {
            const monthlyAllowed = await checkLeave(user, startDate, leaveLabelKeys.paid, settings.monthlyMaxPaidLeave, leaveModel, 'monthly');
            if (!monthlyAllowed) {
                return handleError(res, "Only 2 paid leaves allowed per month.", 400);
            }
            const yearlyAllowed = await checkLeave(user, startDate, leaveLabelKeys.paid, settings.yearlyPaidLeave, leaveModel, 'yearly');
            if (!yearlyAllowed) {
