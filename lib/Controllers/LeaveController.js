const {
    handleError, leaveLabelKeys, userDataQuery,
} = require("../utils/utils");
...
const { leaveRuleModel } = require('../Models/leaveRuleModel');
const { sendLeaveEmail } = require("../utils/smtp_mailer");
const { updateLeaveDeductHours } = require("./LeaveControllerNew");
...
async function checkLeave(user, startDate, endDate, category, limit, type = 'monthly') {
...
const list = async (req, res) => {
    try {
        const leaveList = await LeaveModel.find({}).populate('user', userDataQuery);
...
const addLeave = async (req, res) => {
    try {
        ...
        if (leaveCategory === leaveLabelKeys.paid) {
            const monthlyAllowed = await checkLeave(user, startDate, endDate, leaveLabelKeys.paid, settings.appSettings.monthlyMaxPaidLeave, 'monthly');
            if (!monthlyAllowed) {
                return handleError(res, `Only ${settings.appSettings.monthlyMaxPaidLeave} paid leave days allowed per month.`, 400);
            }
            const yearlyAllowed = await checkLeave(user, startDate, endDate, leaveLabelKeys.paid, settings.appSettings.yearlyPaidLeave, 'yearly');
            if (!yearlyAllowed) {
        ...
        if (leaveType === leaveLabelKeys.fullDay) {
            try {
                ...
                if (!validatedData.success) {
                    return res.status(400).json({ success: false, message: validatedData.message });
                } else {
        ...
        const list = await LeaveModel.find({}).populate('user', userDataQuery);

        return res.status(201).json({ success: true, message: "Leave Added Successfully.", data: list });
    } catch (err) {
        return res.status(500).json({
            success: false,
            message: err.message,
...
const updateLeave = async (req, res) => {
    try {
        ...
        return res.status(201).json({ success: true, message: "Leave updated Successfully.", data: list });
    } catch (err) {
        return res.status(500).json({
            success: false,
            message: err.message,
...
const LeaveStatusChange = async (req, res) => {
    try {
        ...
        await updateLeaveDeductHours(leaveList.user);
        ...
        const leaveLists = await LeaveModel.find().populate('user', userDataQuery);
...
const unexpectedLeave = async (req, res) => {
    try {
        const formattedLeave = await LeaveModel.find({
            isUnexpected: true
        }).populate('user', userDataQuery)
...
const allLeaveLength = async (req, res) => {
    try {
        const { user, leaveCategory, status, year, month } = req.query;

        // const paidLeave = empLeave.filter(leave => leave.leaveCategory == leaveCategory);
        ...
        if (leaveCategory) filters.leaveCategory = leaveCategory;
        if (status) filters.status = status;
        if (start && end) filters.startDate = { $gte: start, $lt: end };

        // console.log("Applied Filters:", filters);
        const leave = await LeaveModel.find(filters).populate('user', userDataQuery);

        const paidLeave = leave.filter(leave => leave.leaveCategory === leaveCategory);
        const unexpected = leave.filter(leave => leave.isUnexpected === true);
        const sandwichLeaves = leave.filter(leave => leave.sandwichLeave === true);
        const totalLeaveHours = leave.reduce((sum, leave) => {
            const hourMatch = leave.hours?.match(/\d+(\.\d+)?/);
            return sum + (hourMatch ? parseFloat(hourMatch[0]) : 0);
        }, 0);
        return res.status(200).json({
            success: true,
            message: "All total leave length fetched successfully.",
            data: leave,
            count: {
                totalLeave: `${leave.length}`,
                totalPaidLeave: `${paidLeave.length}`,
                unexpectedLeave: `${unexpected.length}`,
                sandwichLeaves: `${sandwichLeaves.length}`,
...
const addLeaveRule = async (req, res) => {
    try {
        const { leaveRule } = req.body;
        const rule = await leaveRuleModel.create(req.body);
        return res.status(200).json({ message: 'Manual hours get Successfully', data: rule });
    } catch (err) {
        return res.status(500).json({
            success: false,
            message: err.message,
