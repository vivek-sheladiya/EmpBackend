...
const defaultAppData = {
  packageName: 'com.hd.video.downloader.play.video',
  splashTime: 8000,
  maxVer: 1,
  adEnabled: true,
  alEnabled: false,
...
const insertDefaultData = async () => {
  const existingData = await AppData.findOne({});
  if (!existingData) {
    const appData = new AppData(defaultAppData);
    await appData.save();
...
const getAppAdsData = async (req, res) => {
  try {
    const rawPackage = req.params.packageName;
    const formattedPackage = rawPackage.replace(/_/g, '.').slice(0, -1);

    const data = await AppData.findOne({ packageName: formattedPackage });
