...
//         }
//         // const today = moment();
//         // const allBasicSalaries = await basicSalaryModel.find().populate('user', userDataQuery);
//         // const newReports = [];
//         // const appSettings = await AppSettingModel.findOne();
//         // const monthlyTotalHours = appSettings.monthlyTotalHours;
//
//         const punchReportsList = await punchReportModel.find({}).populate('user', userDataQuery);
...
const getPunchReports = async (req, res) => {
    try {
        ...
        const allPunchReports = await punchReportModel.find(query).populate('user', userDataQuery);
...
const updatePunchReports = async (req, res) => {
    try {
        ...
        doc.user = user;
        await doc.save();
        const dataList = await punchReportModel.find({}).populate('user', userDataQuery);
...
const salaryReportsGenerate = async (req, res) => {
    try {
        ...
        const dataList = await salaryReportModel.find({}).populate('user', userDataQuery);
    ...
    try {
        ...
        for (const rowValues of rawData) {
            ...
            if (rowIndex >= headersRowIndex && rowValues[1]) {
                ...
                const filteredTimeData = allDailyTimes.find(item => item.date === formattedDate);
                const requiredHour = filteredTimeData?.totalHour ?? 0;
        ...
        const list = await punchReportModel.find({}).populate('user', userDataQuery);
...
const addPunchTimeToAllUsers = async (req, res) => {
    try {
        ...
        const requiredHour = allDailyTimes.find(d => d.date === date)?.totalHour ?? 0;

        const allReports = await punchReportModel.find({
            "punchReport.date": date
        });

        const bulkOps = [];

        for (const report of allReports) {
            const punchData = report.punchReport.find(p => p.date === date);
            if (!punchData) continue;

            const punchList = punchData.punchList;

            // Find first null time slot
            const nullIndex = punchList.findIndex(p => p.time === null);
            if (nullIndex === -1) continue;

            // Set the time
            punchList[nullIndex].time = time;

            // Calculate total worked time using available time pairs
            const pairs = [];
            for (let i = 0; i < punchList.length - 1; i += 2) {
                const inTimeStr = punchList[i]?.time;
                const outTimeStr = punchList[i + 1]?.time;

                if (inTimeStr && outTimeStr) {
                    const inDateTime = new Date(`${date}T${inTimeStr}:00`);
                    const outDateTime = new Date(`${date}T${outTimeStr}:00`);
            ...
            if (totalWorkedHours < requiredHour) {
                missingMinutes = Math.round((requiredHour - totalWorkedHours) * 60);
            } else {
                isDeductible = false;
            }
            ...
            punchData.workingHours = formattedWorkingHours;
            punchData.missingHours = formattedMissingTime;
            punchData.isDeductible = isDeductible;
            ...
            bulkOps.push({
                updateOne: {
                    ...
                    update: {
                        $set: {
                            "punchReport.$[outer].punchList": punchList,
        ...
        return res.status(200).json({
            message: `Punch time '${time}' updated on '${date}' for ${bulkOps.length} users.`,
        });
