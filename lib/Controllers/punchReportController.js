    ...
    try {
        ...
        if (existingReport) {
            for (const row of result) {
                ...
                const punchList = row.punchList;
                const workingTime = row.workingTime;
                const missingTime = row.missingTime;
            
                const updated = await punchReportModel.findOneAndUpdate(
                    { empCode: empCode, "punchReport.date": formattedDate },
                    {
                        $set: {
                            "punchReport.$.punchList": punchList,
                            "punchReport.$.workingHours": `${workingTime.hours}:${workingTime.minutes.toString().padStart(2, '0')}`,
            ...
            return res.status(200).json({
                success: true,
                message: "Punch data updated or added successfully.",
