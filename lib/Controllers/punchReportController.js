...
const updatePunchReports = async (req, res) => {
    try {
        ...
        report.workingHours = workingHours || report.workingHours;
        report.missingHours = missingHours || report.missingHours;
        doc.empCode = empCode;
        doc.user = user;
        await doc.save();
        const dataList = await punchReportModel.find({}).populate('user', 'fullName');
        const punchData = dataList.map(punchList => {
            return {
                ...punchList._doc,
                user: {
                    userId: punchList.user._id,
                    fullName: punchList.user.fullName
                }
            };
        });
