    ...
    for (const leave of userLeaves) {
        ...
        for (let date = start.clone(); date.isSameOrBefore(end); date.add(1, 'day')) {
            ...
            plainLeave.deductForDate = {
                date: dateStr,
                deductHours: matchedDateEntry.deductHours ? matchedDateEntry.deductHours : 0,
                deductMinutes: matchedDateEntry.deductMinutes ? matchedDateEntry.deductMinutes : 0
            };
...
async function updateLeaveDeductHours(leaveData) {
    ...
    if (leaveData.sandwichLeave && dateArray.length > 1) {
        if(dateArray[dateArray.length - 1].deductHours) {
            dateArray[dateArray.length - 1].deductHours += 9;
        }
        if(dateArray[dateArray.length - 1].deductMinutes) {
