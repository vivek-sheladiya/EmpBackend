...
const {sendOtpEmail} = require("../utils/smtp_mailer");
const {
    handleError,
    findEmailAddress,
    findMobileNumber,
...
const login = async (req, res) => {
    try {
        ...
        let user = await findEmailAddress(loginId || emailAddress);
        if (!user) return handleError(res, "User Not Found", 400);
        if (user.approvalStatus !== 'Approved') {
            return handleError(res, "Access denied: Your account is pending approval. Please contact your administrator.", 400);
        }
        if (!user.isActive) {
            return handleError(res, "Account inactive: Please contact your administrator to activate your account.", 400);
        }
...
const forgetPassSendOtp = async (req, res) => {
    try {
        ...
        user.otp = otp;
        await user.save();
        await sendOtpEmail(emailAddress, user.fullName, otp);
