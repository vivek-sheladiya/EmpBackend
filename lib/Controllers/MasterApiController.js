const {handleError, UserRole, ApprovalStatus, userDataQuery} = require("../utils/utils");
...
const {getRooms} = require("./ChatControllerNew");
const {generateBirthdayEvents} = require("./HolidayEventController");
...
const getMasterData = async (req, res) => {
    try {
        ...
        const eventsFullRecord = await HolidayEventModel.find({});
        const holidayData = eventsFullRecord.filter(event => event.isLeaveOnDay === true);
        ...
        const chatRoomsData = await getRooms(loginUser._id);
        ...
        if(isAdmin) {
            ...
            const basicSalaryData = await basicSalaryModel.find(undefined, undefined, undefined).populate('user', userDataQuery);
